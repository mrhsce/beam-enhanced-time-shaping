...
from data_density_bs import delays, jitters, queues
...
def plot_metric(metric_data, metric_name, ylabel, output_file):
    ...
    for scenario, scenario_m in zip(scenarios, scenarios_m):
        mean_values = {bs: metric_data[scenario][bs][3] for bs in bs_counts}
        mean_values_m = {bs: metric_data[scenario_m][bs][3] for bs in bs_counts}

        values_to_plot = [mean_values[bs] for bs in bs_counts]
        values_to_plot_m = [mean_values_m[bs] for bs in bs_counts]

        x = np.arange(len(bs_counts))  # the label locations
        ...
        improvements = []
        for bs in bs_counts:
            without_s = mean_values[bs]
            with_s = mean_values_m[bs]
...
plot_metric(delays, 'delay', 'Delay (ms)', 'delays_comparison_density_bs.png')
plot_metric(jitters, 'jitter', 'Jitter (ms)', 'jitters_comparison_density_bs.png')
plot_metric(queues, 'queue length', 'Queue Length', 'queues_comparison_density_bs.png')
