...
def prepare_and_run_simulations(base_dir, mode):
    ...
    utility_functions = [UESelectionUtilityFunction.BITRATE_MAXIMIZATION,
    ...
    for sub_allocation in sub_allocation:
        for utility in utility_functions:
            for seed in random_seeds:
                random.seed(seed)
                np.random.seed(seed)
                policy = Policy(operation_mode=mode, subcoherence_time_allocation=sub_allocation, utility_function=utility)
